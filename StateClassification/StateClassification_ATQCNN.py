...
def ATQCNNcircuit(inputs, QC1, QC2, QC3, QP1, QP2, QP3, QF):
    ...
    for i in range(7):
        qml.CZ(wires=[i, (i + 1) % 6])
    qml.RX(phi=inputs[1], wires=int(inputs[0]))
    # endregion
    ...
    return qml.expval(qml.PauliZ(5)), qml.expval(qml.PauliZ(6))
...
if __name__ == "__main__":
    try:
        with tf.device('/device:GPU:1'):
            train_data, train_label, test_data, test_label = GenerateQuantumState.generate_data(7)
            ...
            for e in range(epoch):
                ...
                with open("ATQCNN_2State_Acc.txt", 'w') as file:

                    file.write(str(test_accuracy_results) + "\n")
                with open("ATQCNN_2State_Loss.txt", 'w') as file:
