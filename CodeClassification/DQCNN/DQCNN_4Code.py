...
def QCNNcircuit(inputs, QC1, QC2, QC3, QC4, QC5,
                QP1, QP2, QP3, QP4, QP5,
                QF):
    # print(inputs.shape)
    start_qubits = 7
...
class MyQCNNLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, start_qubits, final_qubits):
        ...
        self.QF = tf.Variable(tf.zeros(2 * 2 + 2 * 2 * 2, dtype=tf.float64))  # 12
        ...
        for i in range(len(inputs)):
            try:
                out_temp = self.qnode(inputs[i],
                                      self.QC1, self.QC2, self.QC3, self.QC4, self.QC5,
                                      self.QP1, self.QP2, self.QP3, self.QP4, self.QP5,
                                      self.QF)
            except:
                print(inputs[i])
...
if __name__ == "__main__":
    try:
        with tf.device('/device:GPU:1'):
            ...
            MyModel = MyQCNNLayer(start_qubits=7, final_qubits=2)
            epoch = 50
            batch_size = 100
            ...
            for e in range(epoch):
                ...
                with open("DQCNN_4Code_Acc.txt", 'w') as file:

                    file.write(str(test_accuracy_results) + "\n")
                with open("DQCNN_4Code_Loss.txt", 'w') as file:
